...
def find_sift(im_source, im_search, threshold=0.8, rgb=True, good_ratio=FILTER_RATIO):
    ...
    indexParams = dict(algorithm=FLANN_INDEX_composite, trees=4, branching=10, iterations=5)
    ...
    targetIndexPoint = []
    originalIndexPoint = []
    li = np.arange(0.1, 1, 0.01)
    for i in li:
        for j, (m, n) in enumerate(matches):
            if m.distance < i * n.distance:
                targetIndexPoint.append(m.trainIdx)
                originalIndexPoint.append(n.queryIdx)
                good.append([m])
        if len(good):
            break
    (targetx, targety) = kp2[targetIndexPoint[0]].pt
    return (int(targetx),int(targety))
